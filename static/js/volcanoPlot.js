define([''],
    ...
    return Backbone.View.extend({
        ...
        initialize: function(data){
            /*this.render([{"gene" : "C13orf15", "log2(fold_change)" : -3.26837, "q_value" : 2.50041e-13},
        ...
        processData : function(data){
            var IDs = data.IDs;
            var x_vals = data.x_vals;
            var y_vals = data.y_vals;
            var name = data.name;
            var category = data.category;
            var output_data = [];
            for (var i = 0; i < data.IDs.length; i++){
                output_data.push({"go_id" : IDs[i],
                                "category" : category[i],
                                "name" : name[i],
                                "log2(fold_change)" : parseFloat(x_vals[i]),
                                "q_value" : parseFloat(y_vals[i])});
            }
            return output_data;
        },
